...
class ValidateTwilioMiddleware(object):
    ...
    :param app: WSGI app to pass validated requests to.
    :param auth_token: Twilio Auth Token
    :param base_url: If passed use this as the URL to verify against instead
                       of trying to reconstruct the URL.
    """
    ...
    def __call__(self, environ, start_response):
        ...
        if environ['REQUEST_METHOD'] == 'POST':
            try:
                ...
                environ['wsgi.input'] = StringIO(body) # Make sure self.app 
                                                       # can read this
        ...
        self.log.error('Could not validate twilio signature (%s) against ' 
                       'URL %s with POST args: %s', signature, url, args)
        return self.denied(start_response)
    ...
    def build_request_url(self, environ):
        ...
        # Attempt to reconstruct the URL per PEP 333 and 
        # https://www.twilio.com/docs/security
        urlparts = [
    ...
    def parse_query_string(self, query_string):
        ...
        for key, value in parse_qsl(query_string, keep_blank_values=True, 
                                    strict_parsing=True):
            if key in params:
                raise ValueError('Duplicate POST param received from twilio!')
